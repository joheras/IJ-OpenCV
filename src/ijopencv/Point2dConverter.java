 ...
 * @author jonathan
 */
public class Point2dConverter implements IConverter<PointRoi, opencv_core.Point2d>{

    @Override
    public opencv_core.Point2d convertTo(PointRoi pr) {
        if (!canConvertTo(pr)) {
            try {
    ...
    @Override
    public PointRoi convertFrom(opencv_core.Point2d p) {
        if (!canConvertFrom(p)) {
            try {
    ...
    @Override
    public boolean canConvertFrom(opencv_core.Point2d p) {
        if (p.sizeof() != 2) {
            return false;
        }
        return true;
